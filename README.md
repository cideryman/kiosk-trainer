...
- 이용자가 개인별 1회 주문 한도 안에서 간식 선택, 주문 확인과 완료 과정을 반복 연습합니다.
...
관리자·주방·후기 화면은 관리자 인증 게이트를 거친 뒤 데이터를 표시합니다. 일반 키오스크와 공개 배달왔삼 페이지는 운영 목적상 공개 진입 흐름을 유지하며, 주문·재고·일반 주문 한도·배달왔삼 온기는 서버에서 최종 확인합니다.
...
| `20_Users.gs` | 일반 이용자와 개인별 1회 주문 한도 |
...
| 시트 | 용도 |
| --- | --- |
| `이용자목록` | 일반 키오스크 이용자, 닉네임, 개인별 1회 주문 한도(`크레딧` 호환 열), 사용 여부, 이미지 URL |
