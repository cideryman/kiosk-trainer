...
1. `🔍 주방` 탭으로 이동합니다.
2. 관리자 인증을 해제합니다.
3. 운영설정에서 `매주 수요일 자동 운영`과 `13:00~15:00`을 확인합니다. 처음 배포하면 자동 운영은 OFF이므로 사용할 때 ON으로 바꾸고 `정기 일정 저장`을 누릅니다.
4. 현재 운영 출처와 다음 자동 개방 일시를 확인합니다. 정기 운영은 Asia/Seoul 기준으로 수요일 13시에 자동 개방되고 15시에 마감됩니다.
5. 해당 수요일에 운영하지 않으려면 날짜가 표시된 `이번 회차 운영 안 함`을 누릅니다. 중단 중에는 수동 개방 버튼을 사용할 수 없으며, 다시 열려면 먼저 `운영 재개`를 누릅니다.
6. 행사 모드에서는 정기 운영이 자동 개방되지 않습니다. 행사 중 주문을 받을 때는 기존 `20분`, `30분`, `1시간` 또는 직접 운영으로 수동 개방합니다.
7. 자동 일정 외에 임시 운영이 필요하면 기존 수동 운영 버튼을 사용합니다. 정기 운영과 겹치면 더 늦은 마감 시각까지 운영합니다.
...
2. 수요일 자동 운영 중 `지금 마감`을 누르면 그날 회차만 즉시 중단되고 다음 수요일에는 자동으로 다시 열립니다. 즉시 마감에는 진행 중 주문의 5분 완료 유예가 적용되지 않습니다.
3. 15시 자연 마감 때는 15시 전에 시작한 주문만 15시 5분까지 제출할 수 있습니다. 15시부터 새 주문은 시작할 수 없습니다.
