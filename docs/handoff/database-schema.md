...
- **주의**: 아래 행 수는 검증 시점의 스냅샷이며 운영 중 계속 달라질 수 있습니다. 시트명과 열 순서를 구조 기준으로 사용합니다. P104부터 보관 실행 시 `주문내역_자동백업_*`과 기존 `주문보관_자동백업_*` 스냅샷이 함께 생성됩니다. `이용신청_임시백업_*`은 익명화 성공 또는 정상 복구 후 삭제되며 정리·복구 실패 때만 남습니다.
...
| 11 | `이용자목록` | A1:E13 | 5 | 일반 키오스크 이용자와 개인별 1회 주문 한도 |
...
- P103부터 `크레딧` 열은 일반 키오스크의 개인별 1회 주문 한도입니다. 헤더와 API 필드명은 호환성을 위해 유지하며 범위는 1~15, 신규·전환 기본값은 10입니다.
- 일반 주문과 취소는 이 값을 변경하지 않습니다. 배달왔삼의 실제 사용량은 별도 `게스트크레딧` 시트에서 계속 관리합니다.
- 전환 함수는 실행 전에 `이용자목록_한도전환백업_YYYYMMDD_HHMMSS` 숨김 백업 시트를 생성합니다. 실제 실행 전에는 운영 시트 수에 포함하지 않습니다.
...
| K | `orderToken` | 신규 일반·게스트 주문의 UUID 기반 조회·취소 토큰. 기존 게스트 토큰 형식은 호환 |
...
- P105 이후 이용자 직접 취소는 일반·게스트 구분 없이 동일 주문 모든 행의 K열 토큰과 요청 토큰이 정확히 같아야 합니다. 기존 K열이 빈 일반 주문은 관리자 취소만 허용합니다.
- 취소 처리 중에는 `주문내역_취소임시백업_*`, `간식목록_취소임시백업_*`, 게스트 주문이면 `게스트크레딧_취소임시백업_*`을 사용합니다. 성공 또는 정상 자동 복구 후 삭제하며 복구·삭제 실패 때만 남깁니다.
