...
- 기존 `이메일알림큐`를 `notificationType`·`referenceId` 기준으로 확장해 신규 신청 알림을 비동기로 등록하도록 했습니다. 신청 저장은 이메일 큐 실패와 분리됩니다.
...
- 중복 정리와 운영 화면 통합은 GAS 재배포 및 수동 검증 전이어서 당시 P93 완료 상태로 변경하지 않았습니다.
- 이후 GAS 배포와 `setupGuestApplicationOperationsSheet()` 실행은 완료했지만 실제 신규 신청 알림 확인이 남아 있으므로 P93은 활성 우선순위에서 유지합니다.
