...
발달장애인 주간보호센터 이용자가 별명과 개인별 1회 주문 한도로 간식을 주문하고, 주문·준비·배달·수령 과정을 훈련할 수 있도록 만든 PWA 키오스크 및 배달왔삼 시스템입니다. 큰 터치 영역, TTS, 효과음, 진동과 직관적인 한도·온기 표시를 핵심 UX로 유지합니다.
...
- `guest-apply.html`: 봉사자·후원자 대상 배달왔삼 사전 이용 신청. 메뉴 주문과 분리하며 개인정보를 브라우저에 저장하지 않음
...
- `admin.html`: 이용자·간식·1회 주문 한도·표시 순서와 배달왔삼 이용 신청 승인·중지·연락·익명화 관리
...
- 주문·취소·상태 변경 같은 쓰기 경로는 `LockService` 안에서 전체 주문을 검증합니다. 취소는 주문·간식·게스트 지갑 임시 백업 후 배치 저장·재조회 검증하며 실패 시 전부 복원하고, 제공 상태도 같은 주문의 모든 행을 하나의 작업 단위로 변경합니다.
- 주문 생성은 커밋 뒤 주문 잠금을 해제하고 이메일 큐를 별도 문서 잠금으로 적재합니다. `perfDebug: 1` 요청은 개인정보 없는 단계별 `_timings`를 선택적으로 반환합니다.
- 클라이언트 변경 요청이 성공하면 관련 로컬 API 캐시를 무효화합니다.
- 공개 이용 신청은 `submitGuestApplication` POST만 허용하고, 목록·상세·상태·설정·익명화 API는 모두 기존 관리자 토큰으로 보호합니다.
- 이용 신청 승인은 운영 절차이며 기존 배달왔삼 주문 페이지 접근 제어와 연결하지 않습니다.
- 신청 상태 일괄 변경과 운영 일정 확정·완료·취소는 요청 ID를 중복 제거한 뒤 모든 대상을 먼저 검증하고 한 번만 저장합니다. 누락되거나 처리 불가능한 대상이 섞인 요청은 관리자 로그와 결과 캐시를 남기지 않고 전체 거부합니다.
...
| `02_SheetSafety.gs` | 시트 백업·복원·날짜 포함 값 검증·실제 행 구간 저장 |
| `10_KakaoGuests.gs` | 카카오 인증과 게스트 프로필 |
| `11_AdminLog.gs` | 관리자 변경 로그 |
| `12_GuestApplications.gs` | 배달왔삼 이용 신청, 관리자 처리, 신청 설정, 개인정보 익명화 |
| `14_GuestApplicationOperations.gs` | 이용 신청 운영 일정 확정·완료·취소와 요청 멱등성 |
| `20_Users.gs` | 이용자 조회·등록·수정·1회 주문 한도 |
...
| `41_EmailQueue.gs` | 주문·이용신청 이메일 큐 적재, 발송·재시도 |
| `50_Media.gs` | 이미지 URL 변환과 업로드 |
| `60_Settings.gs` | 게스트 운영 설정과 설정 캐시 |
| `61_GuestSchedule.gs` | Asia/Seoul 기준 가변 정기·날짜 지정 추가·긴급 운영·중단 회차 순수 판정 |
...
P99/P102 일정 운영은 시간 트리거를 만들지 않습니다. `getGuestSettings`와 `placeOrder`가 같은 순수 판정 함수에 현재 시각을 주입하며, 주문 저장 직전의 서버 판정이 최종 권한입니다. 행사 모드는 정기·추가 운영을 억제하고 종료 시각 기반 긴급 운영은 유지합니다.
