...
3. **P1 - 브라우저 저장소 개인정보/이용 안내 정비**
   ...
   - **적용 결과**
     - 게스트 주문 마지막 확인 화면의 닉네임·배송지 입력 영역 아래에 `브라우저 저장 안내`를 배치했다.
     - 주문 확인과 크레딧 중복 사용 방지를 위해 주문표시명, 기기 식별값, 주문 조회 정보가 브라우저에 저장된다는 점과 사이트 데이터 삭제 시 함께 삭제된다는 점을 안내한다.
     - 주문을 막는 자동 팝업이나 필수 동의 체크박스는 사용하지 않는다. 사용자가 정보를 입력하는 시점에 상시 보이는 짧은 문구로 고지한다.
     - 아직 날짜 단위 표시명 정책은 구현 전이므로 보관 기간을 `당일`이라고 미리 안내하지 않는다.
     - 정적 파일 반영을 위해 `service-worker.js` 캐시 버전을 `kiosk-cache-v128`로 상향했다.
   - **검증 완료**: 데스크톱·모바일 로컬 화면에서 가로 넘침과 고정 푸터 겹침이 없음을 확인했고, 운영자가 실제 주문 확인 화면의 안내 위치와 문구를 확인해 완료 처리했다.
