...
1. **P3 터치 보조 모드 추가**: 터치가 어려운 이용자를 위한 입력 안정성 개선.
2. **P4 후기 상세 모달 참여 정보 추가 검토**: 후기 참여를 늘리기 위한 관찰 기능. 새 탭/사용자 목록은 만들지 않고, 후기 상세 모달에 해당 작성자의 후기 횟수부터 작게 표시하는 방향을 우선 검토.
...
* **[DONE / P3 2026-06-28] 주방 배달팀 멤버 역할 표시 개선**:
  ...
  - **구현 내용**: DB/설정 저장 형식은 바꾸지 않고, 기존 문자열을 파싱해 `guest.html`의 오늘의 배달팀 표시와 `kitchen.html`의 배달팀 설정 미리보기에서 역할별 카드로 보여주도록 구현했습니다.
  - **결정 이유**: 설정 데이터 구조를 바꾸면 기존 입력값과 저장 로직 영향이 커질 수 있습니다. 표시만 분리하면 운영 가독성은 개선하면서 기존 설정값, GAS, 구글시트 구조는 그대로 유지할 수 있습니다.
  - **캐시 갱신**: `guest.html`, `kitchen.html`, `css/style.css` 변경 반영을 위해 서비스워커 캐시 버전을 `kiosk-cache-v96`으로 올렸습니다.
  - **수동 확인**: `이인, 안태근|배달 담당, 박상민, 김동환|상품 준비 담당` 형식, 역할 일부가 빠진 형식, 일반 자유문 형식을 각각 넣고 게스트 화면과 주방 설정 미리보기가 깨지지 않는지 확인합니다.
...
### 작업 기록 (Development Log) - 27) 배달팀 멤버 역할별 표시 개선

#### 작업명
> 기존 배달팀 문자열 저장 형식을 유지하면서 게스트 화면과 주방 설정에서 역할별 카드로 멤버를 표시
...
* 주방 설정 화면에서는 입력 문자열이 최종 화면에서 어떻게 보일지 바로 확인하기 어려웠습니다.

##### 영향
* 기능 오류는 아니지만, 운영자와 이용자가 “누가 배달 담당이고 누가 상품 준비 담당인지”를 한눈에 읽기 어렵습니다.
* 설정 문자열을 잘못 입력해도 저장 전 시각적 확인이 부족했습니다.

#### 2. Cause (원인 분석)

##### 원인
* 기존 렌더링은 문자열을 역할 그룹으로 묶기는 했지만, 표시 방식이 가로/세로 역할 카드 구조가 아니라 한 줄 항목 중심이었습니다.
* 주방 설정 입력칸에는 파싱 결과를 보여주는 미리보기 영역이 없었습니다.

##### 조사 과정
* `guest.html`의 `renderTodayDeliveryTeam()` 렌더링 로직을 확인했습니다.
* `kitchen.html`의 게스트 운영 설정 패널과 `input-team-members` 저장 흐름을 확인했습니다.
...
* `kitchen.html`에도 같은 파서/렌더러를 추가해 멤버 입력칸 아래에 역할별 미리보기를 표시합니다.
...
* 정적 파일 변경 반영을 위해 `service-worker.js` 캐시 버전을 `kiosk-cache-v96`으로 올렸습니다.

##### 결정 이유
* DB/설정 구조를 바꾸지 않는 것이 가장 안전합니다.
* 기존 `|배달 담당`, `|상품 준비 담당` 형식과 과거 입력값을 그대로 살리면서 표시만 개선할 수 있습니다.
* 주방 설정 미리보기를 추가하면 운영자가 저장 전에 결과를 바로 확인할 수 있습니다.
...
* [x] 정적 분석: 역할 파서/렌더러, 주방 미리보기, 공통 CSS, 서비스워커 캐시 버전 확인
* [x] 빌드 성공: 정적 HTML 앱이라 별도 빌드 없음
* [ ] 콘솔 오류 없음: 실제 브라우저/PWA에서 수동 확인 필요

##### 기능 검증
* [x] 기존 기능 영향 없음: GAS, 구글시트 저장 구조, 게스트 운영 설정 API는 수정하지 않음
* [ ] 신규 기능 정상 동작: 실제 브라우저에서 게스트 화면/주방 설정 미리보기 수동 확인 필요
...
1. `kitchen.html`의 게스트 운영 설정에서 멤버 입력칸에 `이인, 안태근|배달 담당, 박상민, 김동환|상품 준비 담당`을 입력합니다.
2. 입력칸 아래 미리보기가 `배달 담당`, `상품 준비 담당` 카드로 나뉘는지 확인합니다.
3. 저장 후 `guest.html`에서 오늘의 배달팀 섹션도 같은 방식으로 보이는지 확인합니다.
4. `이인, 안태근`처럼 역할이 없는 문자열을 입력했을 때 `멤버` 카드로 안전하게 표시되는지 확인합니다.
5. `이인|배달, 안태근|상품 준비`처럼 다른 역할명이 들어와도 화면이 깨지지 않는지 확인합니다.

##### 기대 결과
* 저장 데이터는 기존 한 줄 문자열 그대로 유지됩니다.
* 화면 표시는 역할별 카드/열로 분리되어 더 쉽게 읽힙니다.
* 역할 구분자가 없는 입력도 깨지지 않고 `멤버`로 표시됩니다.

#### 6. Caution (주의사항)

* GAS 추가 배포 필요 여부: **없음**.
* Service Worker Cache 업데이트 필요 여부: **필요**. `kiosk-cache-v96`으로 올렸습니다.
* DB 컬럼 추가 여부: **없음**.
* 환경설정 변경 여부: **없음**.
...
* 배달팀 멤버가 게스트 화면과 주방 설정 미리보기에서 역할별 카드로 표시됩니다.

##### 왜 바뀌었는가?

* 운영자가 입력 결과를 쉽게 확인하고, 이용자가 오늘의 배달/상품 준비 담당을 더 빠르게 이해할 수 있게 하기 위해서입니다.

##### 운영자가 확인할 것

* GitHub Pages 반영 후 주방 설정 미리보기와 게스트 화면 배달팀 섹션이 역할별로 잘 나뉘는지 확인하세요.
