...
* **[FAILED / IN PROGRESS 2026-06-23] P1 - Board.html dynamic resizing layout and caching issue**:
  ...
  - **현재 상태**: `board.html`의 CSS/JS 동적 레이아웃 수정 및 `service-worker.js` 버전 변경은 완비되었습니다. 깃허브 푸시를 위해 `js/config.js`는 `USE_MOCK = false` 및 기존 목데이터 상태로 다시 롤백(복구)해 두어 실제 API 통신 환경에 영향이 없도록 조치했습니다.
