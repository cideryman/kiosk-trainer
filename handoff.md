...
1. **P3 주방 배달팀 멤버 역할 표시 개선**: 현재 한 줄 텍스트(`멤버: 이인, 안태근|배달 담당, 박상민, 김동환|상품 준비 담당`)를 기존 저장 형식은 유지하되, 주방 화면 표시에서는 `멤버 / 배달 담당 / 상품 준비 담당`처럼 같은 행의 별도 열로 분리해 가독성을 높이는 개선.
2. **P3 터치 보조 모드 추가**: 터치가 어려운 이용자를 위한 입력 안정성 개선.
3. **P4 후기 상세 모달 참여 정보 추가 검토**: 후기 참여를 늘리기 위한 관찰 기능. 새 탭/사용자 목록은 만들지 않고, 후기 상세 모달에 해당 작성자의 후기 횟수부터 작게 표시하는 방향을 우선 검토.
