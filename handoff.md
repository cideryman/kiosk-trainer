...
### Recently Resolved
* **[NEW]** P2 - 빌지 인쇄 페이지 내 일반 빌지 / 애니라벨 V3050 라벨지 선택 인쇄 기능 추가 (Development Log - 50)
...
* P3 - 백엔드 보안 하드닝(P3) 검토 및 보류 결정 (Development Log - 47)
* P3 - 구글 시트 오류 메시지를 사용자 친화적인 '데이터 연결 실패'로 변경 (Development Log - 46)
...
### 작업 기록 (Development Log) - 30) 게스트 취소 및 후기 등록 시 토큰 검증 강화
...
### 작업 기록 (Development Log) - 31) 게스트 후기 사진 업로드 가드레일 및 규격 제한 적용
...
### 작업 기록 (Development Log) - 32) 주방 신규 주문 감지 시 화면 필터 우회 수정
...
### 작업 기록 (Development Log) - 33) 관리자 운영 점검 모달 반응형 레이아웃 강화
...
### 작업 기록 (Development Log) - 34) 주방 화면 주문 통계 중복 카운트 버그 수정
...
### 작업 기록 (Development Log) - 36) 관리자 화면별 상단 색 구분 및 PWA theme-color 연동
...
### 작업 기록 (Development Log) - 37) 이용자 및 간식 관리 탭 1열 터치 클릭 모달 연동
...
### 작업 기록 (Development Log) - 48) admin화면 이용자 크레딧 단축 버튼 추가 및 API 요청 시 행 단위 락 처리
...
### 작업 기록 (Development Log) - 49) 카카오 연동 게스트 (비회원) 문구 생략 및 말풍선(💬) 이모지 표시 연동
...
### 작업 기록 (Development Log) - 50) 빌지 인쇄 페이지 내 일반 빌지 / 애니라벨 V3050 라벨지 선택 인쇄 기능 추가
